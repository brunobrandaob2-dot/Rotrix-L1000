...
fn default_update_checks_enabled() -> bool {
    // Rotrix: sem busca de atualizacao do Handy original
    false
}
...
fn default_word_correction_threshold() -> f64 {
    // Rotrix: mesmo valor usado no sistema atual (vocabulario grande de radiologia)
    0.08
}
...
fn default_post_process_enabled() -> bool {
    // Rotrix: o pos-processamento e o Roteador de Laudos local
    true
}
...
fn default_post_process_providers() -> Vec<PostProcessProvider> {
    ...
    providers.push(PostProcessProvider {
        id: "custom".to_string(),
        label: "Roteador Rotrix (local)".to_string(),
        base_url: ROTRIX_ROUTER_URL.to_string(),
...
/// Rotrix: endereco do Roteador de Laudos que o app sobe junto (rotrix.rs).
pub const ROTRIX_ROUTER_URL: &str = "http://127.0.0.1:8123/v1";
pub const ROTRIX_PROMPT_ID: &str = "roteador_local";

/// Rotrix: vocabulario de radiologia (pt-BR) para o reconhecimento de voz.
fn rotrix_custom_words() -> Vec<String> {
    include_str!("../resources/rotrix_custom_words.txt")
        .lines()
        .map(|l| l.trim())
        .filter(|l| !l.is_empty())
        .map(|l| l.to_string())
...
fn default_post_process_prompts() -> Vec<LLMPrompt> {
    vec![LLMPrompt {
        id: ROTRIX_PROMPT_ID.to_string(),
        name: "Roteador local".to_string(),
...
pub fn get_default_settings() -> AppSettings {
    ...
    AppSettings {
        ...
        custom_words: rotrix_custom_words(),
        ...
        post_process_selected_prompt_id: Some(ROTRIX_PROMPT_ID.to_string()),
